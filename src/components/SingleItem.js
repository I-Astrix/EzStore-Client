...
const SingleItem = () => {
    ...
    // console.log(currentProduct.mainimg.data.attributes.url);
    
    
     async function getById(){
      isLoading.current = true;
      const request = await fetch(`http://localhost:1337/api/products/6?populate=*`);

      
      const response = await request.json();

      setCurrentProducts(response.data.attributes);
      // console.log(response.data.attributes.mainimg.data.attributes.url)
      setCurrentImages(response.data.attributes.mainimg.data.attributes.url)
  ...
  <Link to="/" className="flex gap-2 font-semibold text-indigo-600 text-sm items-center mb-5 hover:text-gray-400">
    ...
    <path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" />
     ...
     {!isLoading === false && <div className="grid grid-cols-1 gap-y-10 sm:grid-cols-2 gap-x-6">
          ...
          {/* Left Container */}
              ...
              <div className="flex flex-col">
                  <Animate>
                  <img src={currentImages} alt="" className='scale-2 h-max object-cover rounded-2xl shadow-md transition-all duration-300 ease'/>
                  </Animate>
                  <div className="grid grid-cols-4 gap-5 my-5">
  
                  {/* {images.length > 1 && images.map(image=>{
                    return (
                      <img src={image} key={image} onClick={()=> setMainImage(image)} alt="" className=' rounded shadow-md '/>
                    )
                  })}   */}
          ...
          {/* Right Containers */}
              <div className="">
              <h1 className='text-5xl mb-5 font-thin w-wrap '>{}</h1>
      ...
      <p className='font-thin text-md'>Rated {} On EzStore</p>
              ...
              <h1 className='text-5xl font-thin my-5'>₹. {} <span className="text-sm">*Price May vary</span></h1>
